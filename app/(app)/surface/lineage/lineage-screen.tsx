...
import { lineageQuery, observationsQuery, subjectsQuery } from "../../_queries";
...
export function LineageScreen() {
  ...
  const observation = params.get("observation") ?? undefined;

  const subjectsQ = useQuery({
    queryKey: keys.observed.subjects(workspace ?? ""),
    queryFn: () => subjectsQuery(workspace!),
  ...
  const observationsQ = useQuery({
    queryKey: keys.observed.observations(workspace ?? "", chosen ?? ""),
    queryFn: () => observationsQuery(workspace!, chosen!),
  ...
  const lineageQ = useQuery({
    queryKey: keys.observed.lineage(workspace ?? "", walked ?? ""),
    queryFn: () => lineageQuery(workspace!, walked!),
    enabled: Boolean(workspace && walked),
  });
  ...
  if (!workspace) {
    return (
      <>
        ...
        <NoWorkspace what="An observation" />
      </>
    );
  }
