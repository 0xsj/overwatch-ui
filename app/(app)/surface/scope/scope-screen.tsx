...
import { rulesQuery, targetsQuery } from "../../_queries";
import { RuleForm } from "../_components/rule-form";
import { SupersedeButton } from "../_components/supersede-button";
...
export function ScopeScreen() {
  ...
  const rulesQ = useQuery({
    queryKey: keys.targets.scope(workspace ?? "", chosen ?? "", all === "1"),
    queryFn: () => rulesQuery(workspace!, chosen!, all === "1"),
  ...
  return (
    <>
      ...
      {targets.length === 0 ? (
        <Alert tone="info">
          <Text size="sm">
            No targets in this engagement. Scope hangs off a target rather than
      ...
      {chosen ? (
        <Panel
          title="Write a rule"
          note="Nothing is in scope until a rule says so — that is the default, and it is why a first run refuses everything with no rule to cite."
        >
          <RuleForm workspaceId={workspace} targetId={chosen} />
        </Panel>
      ) : null}
