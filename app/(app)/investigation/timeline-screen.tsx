...
import { eventHref, eventRelationshipHref, eventRelationshipQuestionDraft, eventRevisionHref } from "@/lib/services/events/navigation";
...
export function TimelineScreen({ workspace }: { workspace: string }) {
  ...
  const focusedRelationshipID = searchParams.get("relationship")?.trim() ?? "";
...
function EventClusterPanel({ workspace, events, clusters, mayWrite, error, moreAvailable, morePending, loadMore }: { workspace: string; events: TimelineEvent[]; clusters: EventCluster[]; mayWrite: boolean; error: Error | null; moreAvailable: boolean; morePending: boolean; loadMore: () => void }) {
  ...
  return <Panel title="Event hypotheses" note="Group possible same-event accounts without merging events or asserting causality.">
    <div className={s.stack}>
      ...
      {mayWrite ? <details className={s.details}><summary>Group events into a hypothesis</summary><form className={s.stack} aria-label="Group events into a hypothesis" onSubmit={(form) => { form.preventDefault(); create.mutate(); }}>
        ...
        <div className={s.details}><Text size="sm">Events in hypothesis <span className={s.muted}>({selected.length}/12)</span></Text><div className={s.checkboxList}>{events.map((event) => { const checked = selected.includes(event.event_id); const disabled = !checked && selected.length >= 12; return <label key={event.event_id} className={s.checkboxLabel}><input type="checkbox" aria-label={`Include ${event.title} in this hypothesis`} checked={checked} disabled={disabled} onChange={() => setSelected(checked ? selected.filter((id) => id !== event.event_id) : [...selected, event.event_id])} /><span className={s.questionLinkText}>{event.title} <span className={s.muted}>{event.reported_time || "time unknown"}</span></span></label>; })}</div></div>
...
function EventRelationshipPanel({ workspace, events, relationships, focusedRelationshipID, evidence, evidenceError, evidenceHasNext, evidenceFetchingNext, fetchMoreEvidence, mayWrite, error, moreAvailable, morePending, loadMore }: { workspace: string; events: TimelineEvent[]; relationships: EventRelationship[]; focusedRelationshipID: string; evidence: Evidence[]; evidenceError: Error | null; evidenceHasNext: boolean; evidenceFetchingNext: boolean; fetchMoreEvidence: () => void; mayWrite: boolean; error: Error | null; moreAvailable: boolean; morePending: boolean; loadMore: () => void }) {
  ...
  return <Panel title="Event relationships" note="Record authored sequence and related-event hypotheses without inferring causality.">
    <div className={s.stack}>
      ...
      <Failure error={error} />
      {relationships.length ? <div className={s.stack}>{relationships.map((relationship) => <EventRelationshipCard key={relationship.relationship_id} workspace={workspace} relationship={relationship} focused={relationship.relationship_id === focusedRelationshipID} evidence={evidence} evidenceError={evidenceError} fromTitle={eventNames.get(relationship.from_event_id) ?? relationship.from_event_id} toTitle={eventNames.get(relationship.to_event_id) ?? relationship.to_event_id} mayWrite={mayWrite} />)}</div> : <Text size="sm" tone="tertiary">No event relationships have been recorded.</Text>}
      <MoreButton available={moreAvailable} pending={morePending} load={loadMore} />
      {mayWrite ? <details className={s.details}><summary>Record an event relationship</summary><form className={s.stack} aria-label="Record an event relationship" onSubmit={(form) => { form.preventDefault(); create.mutate(); }}>
...
function EventRelationshipCard({ workspace, relationship, focused, evidence, evidenceError, fromTitle, toTitle, mayWrite }: { workspace: string; relationship: EventRelationship; focused: boolean; evidence: Evidence[]; evidenceError: Error | null; fromTitle: string; toTitle: string; mayWrite: boolean }) {
...
function EventEditor({ workspace, event, evidence, evidenceError, evidenceHasNext, evidenceFetchingNext, fetchMoreEvidence, records, recordsError, recordsHasNext, recordsFetchingNext, fetchMoreRecords, history, historyError, accounts, accountsError, mayWrite, shell, saved }: { workspace: string; event?: TimelineEvent; evidence: Evidence[]; evidenceError: Error | null; evidenceHasNext: boolean; evidenceFetchingNext: boolean; fetchMoreEvidence: () => void; records: ResearchRecord[]; recordsError: Error | null; recordsHasNext: boolean; recordsFetchingNext: boolean; fetchMoreRecords: () => void; history: TimelineEventRevision[]; historyError: Error | null; accounts?: EventAccountPage; accountsError: Error | null; mayWrite: boolean; shell?: ReturnType<typeof useContext>["shell"]; saved: (event: TimelineEvent) => void }) {
  ...
  return <div className={s.stack}><form className={s.eventEditor} aria-label={event ? "Edit reported event" : "Record an event"} onSubmit={(form) => { form.preventDefault(); save.mutate(); }}>
...
function EventAccounts({ workspace, event, page, error, evidence, mayWrite }: { workspace: string; event: TimelineEvent; page?: EventAccountPage; error: Error | null; evidence: Evidence[]; mayWrite: boolean }) {
  ...
  return <Panel title="Competing source accounts" note="Each account remains separate until an analyst records an explicit reconciliation.">
    <div className={s.stack}>
      ...
      {mayWrite ? <>
        <details className={s.details}><summary>Add a competing source account</summary><form className={s.stack} aria-label="Add a competing source account" onSubmit={(form) => { form.preventDefault(); create.mutate(); }}>
