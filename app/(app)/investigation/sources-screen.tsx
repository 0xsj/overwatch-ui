...
export function SourcesScreen({ workspace }: { workspace: string }) {
  ...
  return <>
    ...
    <Panel title="AI provider policy" note="A workspace-wide privacy boundary for retained research material and synthesis.">
      <Query of={assistancePolicy} label="assistance provider policy">{(policy) => <div className={s.stack}>
        <div className={s.row}><Badge tone={policy.allow_external ? "warn" : "accent"}>{policy.allow_external ? "External AI allowed" : "External AI blocked"}</Badge><Text size="sm">{policy.allow_external ? "Retained material may be sent to an explicitly configured external provider for bounded assistance and synthesis." : "Only local assistance may use retained material until an admin opts in."}</Text></div>
    ...
    <Panel title="Source intake review" note="Stage URLs or imported files here before they become retained source records. Pending files stay outside source history until a writer approves them.">
      <div className={s.stack}>
        {mayWrite ? <form className={s.stack} aria-label="Stage a source candidate" onSubmit={(event) => { event.preventDefault(); createIntake.mutate(); }}>
