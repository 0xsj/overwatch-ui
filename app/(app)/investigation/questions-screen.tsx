...
export function QuestionsScreen({ workspace }: { workspace: string }) {
  ...
  return <>
    <PageHead title="Open questions" actions={mayWrite ? <Button type="button" intent="primary" onClick={() => setActiveId(null)}>New question</Button> : undefined}>
        ...
        <Query of={questions} label="investigation questions">{() => <div className={s.stack}>
          {!rows.length ? (
            <div className={s.empty}>
              <Text size="sm">No questions recorded yet.</Text>
              ...
              {mayWrite ? <Button type="button" intent="primary" onClick={() => setActiveId(null)}>Record a question</Button> : null}
            </div>
          ) : (
            <div className={s.stack}>
              ...
              <Text size="xs" tone="tertiary">Showing {visibleRows.length} of {rows.length} loaded question{rows.length === 1 ? "" : "s"}.</Text>
  ...
  return <article className={active ? s.questionCard + " " + s.questionCardActive : s.questionCard}>
    <button type="button" className={s.questionCardSelect} onClick={select}>
      <span className={s.questionStateRow}>
        <Badge tone={question.state === "open" ? "warn" : question.state === "answered" ? "accent" : "neutral"}>{stateLabels[question.state]}</Badge>
...
function QuestionEditor({ workspace, question, evidence, evidenceError, evidenceHasNext, evidenceFetchingNext, fetchMoreEvidence, mayWrite, shell, saved }: {
  workspace: string;
  question?: InvestigationQuestion;
  evidence: Evidence[];
  ...
  const [observationIds, setObservationIds] = useState<string[]>(question?.observation_ids ?? []);
  ...
  if (!mayWrite) {
    return question ? <QuestionDetail question={question} evidence={evidence} workspace={workspace} shell={shell} evidenceError={evidenceError} /> : <Text size="sm" tone="tertiary">This investigation is read-only. Existing questions remain visible, but new questions require write access.</Text>;
  }
  ...
  return <form className={s.questionEditor} onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
    ...
    <CitationPicker workspace={workspace} label="Cited observations" selected={observationIds} evidence={evidence} setSelected={setObservationIds} error={evidenceError} max={8} hasNext={evidenceHasNext} fetchingNext={evidenceFetchingNext} fetchMore={fetchMoreEvidence} returnTo={investigationPath(workspace, "questions")} />
...
function QuestionDetail({ question, evidence, workspace, shell, evidenceError }: {
  question: InvestigationQuestion;
  evidence: Evidence[];
  workspace: string;
  shell?: ReturnType<typeof useContext>["shell"];
  ...
  return <div className={s.stack}>
    <div className={s.questionStateRow}><Badge tone={question.state === "open" ? "warn" : question.state === "answered" ? "accent" : "neutral"}>{stateLabels[question.state]}</Badge><span className={s.muted}>Updated by {authorLabel(question.updated_by, shell)} · {dateLabel(question.updated_at)}</span></div>
