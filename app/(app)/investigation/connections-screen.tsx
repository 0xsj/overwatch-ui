...
function ConnectionReviewPanel({ workspace, connection, reviews, reviewsError, reviewsHasNext, reviewsFetchingNext, fetchMoreReviews, mayWrite, evidence, evidenceError }: { workspace: string; connection: ResearchConnection; reviews: ResearchConnectionReview[]; reviewsError: Error | null; reviewsHasNext: boolean; reviewsFetchingNext: boolean; fetchMoreReviews: () => void; mayWrite: boolean; evidence: Evidence[]; evidenceError: Error | null }) {
  ...
  return <section className={s.briefSection}>
    ...
    {reviews.length ? <label className={s.row}><Text as="span" size="sm">Saved review</Text><select aria-label="Saved assisted connection review" className={s.select} value={selected?.connection_review_id ?? ""} onChange={(event) => setSelectedID(event.target.value)}>{reviews.map((review) => <option key={review.connection_review_id} value={review.connection_review_id}>{dateLabel(review.created_at)} · {review.findings.length} finding{review.findings.length === 1 ? "" : "s"} · {connectionReviewStatusLabel(review.status)}</option>)}</select></label> : null}
