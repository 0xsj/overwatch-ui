...
import type { ChainStep } from "@/lib/services/ledger";
import { chainOfActsQuery } from "../../../_queries";
...
export function ChainScreen() {
  const correlation = String(useParams().correlation ?? "");
  const q = useQuery({
    queryKey: keys.ledger.chain(correlation),
    queryFn: () => chainOfActsQuery(correlation),
  });
  const steps: ChainStep[] = q.data ?? [];
