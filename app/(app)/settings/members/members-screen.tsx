...
import { workspaceMembersQuery, workspacesQuery } from "../../_queries";
...
export function MembersScreen() {
  ...
  /* How many engagements each person is on, so the removal confirmation can
     name the cost.
     
     Counted from the LISTING and not from `/v1/me`, which is the correction
     that matters: `/v1/me` excludes closed engagements, and removal deletes
     grants on those too. Counting from the switcher's source undercounts, and
     the number in that dialog is the whole reason the dialog exists.

     One query per engagement, keyed per engagement. There is no org-wide grants
     read by design — a workspace the caller cannot see 404s rather than
     appearing empty. Fine at a firm's worth of engagements and wrong at a
     thousand; the answer then is an endpoint, not a cache here. */
  const everyQ = useQuery({
    queryKey: keys.tenancy.workspaces(org ?? ""),
    queryFn: () => workspacesQuery(org!),
    enabled: Boolean(org && manages),
  });
  const seatLists = useQueries({
    queries: (everyQ.data ?? []).map((w) => ({
      queryKey: keys.access.grants(w.workspace_id),
      queryFn: () => workspaceMembersQuery(w.workspace_id),
      enabled: Boolean(manages),
    })),
  });
  ...
  for (const q of seatLists)
    for (const row of q.data ?? []) seats.set(row.account_id, (seats.get(row.account_id) ?? 0) + 1);
    ...
    <>
      ...
      {manages ? (
        ...
        >
          ...
          <InviteForm orgId={org} workspaces={shell.context?.org.workspaces ?? []} />
        </Panel>
      ) : (
        <Alert tone="info">
          <Text size="sm">
