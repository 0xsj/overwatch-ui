...
export const keys = {
  ...
  records: {
    ...
    list: (workspace: string, query = "", kind = "", citation = "", resolution = "", archived = "active") => query.trim() || kind || citation || resolution || archived !== "active" ? ["research-records", workspace, "list", query.trim(), kind, citation, resolution, archived] as const : ["research-records", workspace, "list"] as const,
    ...
    revisions: (workspace: string, record: string) => ["research-records", workspace, "revisions", record] as const,
    neighborhood: (workspace: string, record: string, depth = 1, limit = 50, kind = "", recordKind = "") => ["research-records", workspace, "neighborhood", record, depth, limit, kind, recordKind] as const,
  ...
  connections: {
    ...
    list: (workspace: string, state = "", review = "", query = "", kind = "", recordKind = "") => state || review || query.trim() || kind || recordKind ? ["research-connections", workspace, "list", state, review, query.trim(), kind, recordKind] as const : ["research-connections", workspace, "list"] as const,
