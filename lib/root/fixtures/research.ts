...
import type { AddObservation, AddSource, ArtifactCleanupCandidate, ArtifactCleanupReview, ArtifactCleanupReviewSummary, ArtifactCleanupSweepRun, ArtifactLifecycleStatus, Capture, ManualObservation, MediaType, RetentionQueueState, SetSourcePrivacy, SetSourceRetention, SourceExtraction, SourceRetentionReview, SourceSearchResult, SourceSummary, TextCapture } from "@/lib/services/sources";
import type { WorkingNote } from "@/lib/services/notes";
import type { Evidence, EvidenceRelation, EvidenceSynthesis, RelationKind, SetEvidenceRelation } from "@/lib/services/review";
import type { InvestigationQuestion, QuestionState, WriteQuestion } from "@/lib/services/questions";
import type { AssistanceDetail, AssistanceProposal } from "@/lib/services/assistance";
import type { TimelineEvent, EventTimePrecision, WriteEvent } from "@/lib/services/events";
...
const store: Store = fixtureGlobal.__owFixtureResearch ??= {
  sources: new Map(), captures: new Map(), extractions: new Map(), observations: new Map(), notes: new Map(), relations: new Map(), syntheses: new Map(), questions: new Map(), assistance: new Map(), events: new Map(), briefs: new Map(), snapshots: new Map(), records: new Map(), connections: new Map(), revisions: new Map(), resolutions: new Map(), cleanedRefs: new Set(), cleanupRuns: new Map(), cleanupReviews: new Map(), cleanupReviewHistory: new Map(), sequence: 0,
};
...
async function captureFor(source: SourceSummary, content: string, media_type: MediaType, author: string) {
  ...
  const capture: Capture = {
    ...
    media_type, sha256: Array.from(new Uint8Array(digest), (x) => x.toString(16).padStart(2, "0")).join(""),
...
async function captureBinaryFor(source: SourceSummary, content: Uint8Array, media_type: MediaType, author: string) {
  const signature = media_type === "application/pdf" ? new TextDecoder().decode(content.slice(0, 5)) === "%PDF-"
    : media_type === "image/png" ? Array.from(content.slice(0, 8)).join(",") === "137,80,78,71,13,10,26,10"
      : media_type === "image/jpeg" ? content[0] === 0xff && content[1] === 0xd8 && content[2] === 0xff
        : media_type === "image/webp" ? new TextDecoder().decode(content.slice(0, 4)) === "RIFF" && new TextDecoder().decode(content.slice(8, 12)) === "WEBP"
          : false;
...
const questionStates: QuestionState[] = ["open", "answered", "dismissed"];
...
function briefDraft(workspace: string, body: Partial<WriteBrief>) {
  ...
  return { title, question, current_account, alternatives, limitations, next_steps, observation_ids: observations, question_ids: questions, connection_ids: connections };
}
...
function recordDraft(workspace: string, body: Partial<WriteResearchRecord>) {
  ...
  return { kind, name, description, observation_ids: unique };
}
...
function eventDraft(workspace: string, body: Partial<WriteEvent>) {
  ...
  const known = observationsIn(workspace);
  if (unique.some((id) => !known.some((one) => one.observation_id === id))) throw missing();
  return { title, description, reported_time, time_precision, sort_date, location, observation_ids: unique };
}
...
function assistanceDetail(workspace: string, sourceId: string, capture: Capture, author: string, extractionId?: string): AssistanceDetail {
  ...
  const operation = { operation_id: operationId, workspace_id: workspace, source_id: sourceId, capture_id: capture.capture_id, ...(extractionId ? { extraction_id: extractionId } : {}), status: "completed" as const, provider: "local", method: "sentence-passages-v1", created_by: author, created_at: new Date().toISOString(), completed_at: new Date().toISOString(), proposal_count: proposals.length };
...
function questionDraft(workspace: string, body: Partial<WriteQuestion>) {
  ...
  if (!questionStates.includes(state)) throw invalid("Choose open, answered, or dismissed.");
  if (state !== "open" && !resolution) throw invalid("An answered or dismissed question needs a resolution.");
...
export const researchRoutes: MemoryRoute[] = [(req) => {
  ...
  if (connectionMatch) {
    ...
    if (req.method === "GET") return page(connections, req, (one) => one.connection_id);
    const draft = connectionDraft(workspace, (req.body ?? {}) as Partial<WriteResearchConnection>);
    if (req.method === "POST" && !connectionId) {
      const now = new Date().toISOString();
      const fresh: ResearchConnection = { connection_id: nextId(), workspace_id: workspace, ...draft, author, updated_by: author, created_at: now, updated_at: now };
    ...
    if (req.method === "PUT" && connectionId) {
      ...
      const updated: ResearchConnection = { ...existing, ...draft, updated_by: author, updated_at: new Date().toISOString() };
  ...
  if (recordMatch) {
    ...
    if (req.method === "GET") return page(records, req, (one) => one.record_id);
  ...
  if (snapshotMatch) {
    ...
    const records = store.records.get(workspace) ?? [];
    if (linkedQuestions.some((one) => !one)) throw missing();
    const now = new Date().toISOString();
    const frozen: BriefSnapshot = {
      ...
      observation_ids: [...brief.observation_ids], questions: linkedQuestions.map((one) => ({ question_id: one!.question_id, question: one!.question, state: one!.state, resolution: one!.resolution, observation_ids: [...one!.observation_ids] })), connections: linkedConnections.map((one) => { const from = records.find((record) => record.record_id === one!.from_record_id); const to = records.find((record) => record.record_id === one!.to_record_id); return { connection_id: one!.connection_id, from_record_id: one!.from_record_id, from_record_kind: from?.kind ?? "", from_record_name: from?.name ?? one!.from_record_id, from_record_description: from?.description ?? "", from_record_observation_ids: [...(from?.observation_ids ?? [])], to_record_id: one!.to_record_id, to_record_kind: to?.kind ?? "", to_record_name: to?.name ?? one!.to_record_id, to_record_description: to?.description ?? "", to_record_observation_ids: [...(to?.observation_ids ?? [])], kind: one!.kind, state: one!.state, rationale: one!.rationale, supporting_observation_ids: [...one!.supporting_observation_ids], opposing_observation_ids: [...one!.opposing_observation_ids] }; }),
  ...
  if (generateMatch) {
    ...
    const body = (req.body ?? {}) as { extraction_id?: string };
  ...
  if (questionMatch) {
    ...
    if (req.method === "GET") return page(questions, req, (one) => one.question_id);
  ...
  const lifecycle = /^\/workspaces\/([^/]+)\/sources\/([^/]+)\/(privacy|retention-review|purge)$/.exec(req.path);
  if (lifecycle) {
  ...
  if (area === "notes") {
    ...
    if (req.method === "GET" && !sourceId) return notes.slice(0, 100).map((note) => ({ ...note, mine: note.author === author }));
    ...
    if (req.method === "POST" && !sourceId) {
      const now = new Date().toISOString();
      const note: WorkingNote = { note_id: nextId(), body, author, created_at: now, updated_at: now, edited: false, mine: true };
  ...
  if (!sourceId) {
    ...
    if (req.method === "POST") {
      ...
      const source: SourceSummary = {
        ...
        created_by: author, created_at: new Date().toISOString(), sensitivity: "internal", legal_hold: false, latest_capture: null,
      };
      return (async () => {
        if (retained) {
